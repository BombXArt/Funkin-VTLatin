...
local daDialogueType = 'end'					--'end' for a dialogue at the end of the song, anything else for normal dialogue
...
doCutscene = false			--(only for normal textboxes) [if you want a video at the start of the dialogue disable this and add a video in the first page]

onlyOneTime = true			--make it so the dialogue only appears one time in the song even if you restart the song (only for normal textboxes)
onlyForStoryMode = true
...
music = ''			--'' for no music (you can play music from the 'music' or 'sounds' folders) [you can also change these by using these variables in a dialogue]
musicLoops = true
musicVolume = 0.6
...
characterFiles = {'rakky', 'hetto', 'rev', 'others'}		--add your character file names here

dialogueBoxes = {'vtl', 'invis'}		--add the dialogue boxes here

dialogueName = {'endsong'}		--add the dialogue name here (only one dialogue)
...
function updateDialogueBox()
	...
	if dialogue[page].textColor == '' or dialogue[page].textColor == nil then
		setProperty('doofTxt.color', getColorFromHex('FFFFFF'))
	else
